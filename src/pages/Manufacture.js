...
const styles = {
  ...
  tableHeader: { backgroundColor: "#f8f9fa", color: "#2c3e50", fontWeight: "600" }
};
...
const Manufacture = () => {
  ...
  // For Scrap/Destroy action modal:
  const [actionModalVisible, setActionModalVisible] = useState(false);
  const [actionType, setActionType] = useState(""); // "Scrap" or "Destroy"
  ...
  // ---------- Fetch Manufacture Records from the Manufacture Documents Endpoint ----------
  const fetchManRecords = useCallback(async (currentPage = 1) => {
    try {
      ...
      // Manufacture endpoint – note the different base path:
      const url = `https://saleco.ruu-d.com/manufacturer/documents${params.toString() ? "?" + params.toString() : ""}`;
      const response = await fetch(url, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      });
      if (!response.ok) throw new Error("Failed to fetch Manufacture documents");
      const data = await response.json();
      setManRecords(data);
      setTotalItems(data.length);
      setPage(currentPage);
    } catch (error) {
      console.error("Error fetching Manufacture records:", error);
      alert("Failed to fetch Manufacture records.");
    } finally {
  ...
  useEffect(() => {
    fetchUserDetails();
    fetchManRecords(1);
  }, [fetchUserDetails, fetchManRecords]);
  ...
  // Receives the action ("Scrap" or "Destroy") and the group of records
  const openActionModal = (action, records) => {
    setSelectedSerial(records[0].sn_number);
    setActionType(action);
    setActionMemo(""); // Clear previous memo
    setActionModalVisible(true);
  };

  // ---------- Function to handle Action Modal submission ----------
  const handleActionSubmit = async () => {
    try {
      // Here we use a new endpoint for Manufacture status update with memo.
      const response = await fetch("https://saleco.ruu-d.com/manufacture/status-with-memo", {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          ...
          status: actionType, // "Scrap" or "Destroy"
          memo: actionMemo    // Memo provided by the user
        })
      });
      const data = await response.json();
      if (!response.ok) {
        ...
        alert(`Serial ${selectedSerial} updated to ${actionType}`);
        setActionModalVisible(false);
        fetchManRecords(page);
      }
    } catch (error) {
  ...
  // ---------- Render a Document Card for Manufacture Documents ----------
  const renderDocGroup = (documentId, records) => {
    // Destructure fields from the first record. Also include the action memo.
    const {
      name,
      product_id,
      timestamp,
        ...
        <Card.Body style={{ backgroundColor: "#fff", padding: "1.25rem" }}>
          <Row>
            <Col md={6}>
              ...
              {/* Display the action memo (Manufacture-specific) */}
    ...
    <div style={styles.pageContainer}>
      <Container className="py-4">
        <div style={{ marginBottom: "2rem" }}>
          <h2 style={styles.headerTitle}>Manufacture</h2>
        </div>
        <Row>
          <Col md={3}>
          ...
          <Col md={9}>
            ...
            {loadingRecords ? (
              ...
              <>
                {groupAndSlice(manRecords).map(([documentId, records]) =>
      ...
      {/* Action Modal for Scrap/Destroy */}
